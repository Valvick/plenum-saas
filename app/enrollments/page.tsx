...
export default function EnrollmentsPage() {
  ...
  const supabase = useMemo(() => {
    ...
    try {
      ...
      console.error('Erro criando o client do Supabase:', e);
      return null;
    }
  }, [url, anon]);
  ...
  useEffect(() => {
    (async () => {
      try {
        if (!supabase) {
          setErr('Variáveis do Supabase ausentes. Configure NEXT_PUBLIC_SUPABASE_URL e NEXT_PUBLIC_SUPABASE_ANON_KEY.');
          return;
        }
  ...
  if (!url || !anon) {
    return (
      <div className="p-6">
        <b>Configuração pendente:</b> defina <code>NEXT_PUBLIC_SUPABASE_URL</code> e
        {' '}<code>NEXT_PUBLIC_SUPABASE_ANON_KEY</code> na Vercel (Project → Settings → Environment Variables).
      </div>
    );
  }
